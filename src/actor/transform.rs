...
impl super::Actor {
    ...
    pub fn add_rotation(&self, asset: &mut Asset, diff: glam::Vec3) {
        let Some(norm) = asset.exports[self.transform].get_normal_export_mut()
        else {
            return;
        };
            ...
            Some(scale) => {
                if let Property::StructProperty(struc) = scale {
                    if let Property::RotatorProperty(vec) = &mut struc.value[0] {
                        vec.value.x.0 += diff.x;
                        vec.value.y.0 += diff.y;
                        vec.value.z.0 += diff.z;
                    }
                }
            }
            None => norm
                ...
                .push(Property::StructProperty(StructProperty {
                    ...
                    value: vec![Property::RotatorProperty(RotatorProperty {
                        ...
                        property_guid: None,
                        duplication_index: 0,
                        value: Vector::new(diff.x.into(), diff.y.into(), diff.z.into()),
                    })],
                })),
        }
    }
    ...
    pub fn add_scale(&self, asset: &mut Asset, diff: glam::Vec3) {
        let Some(norm) = asset.exports[self.transform].get_normal_export_mut()
        else {
            return;
        };
